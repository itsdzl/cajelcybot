...
def load_kbbi_word(length=None):
    """Mengambil 1 kata acak dari dataKBBI.json dengan filter panjang huruf yang aman"""
    try:
        with open("dataKBBI.json", "r", encoding="utf-8") as f:
            words = json.load(f)
            # Pastikan format data adalah dict atau list
            word_list = list(words.keys()) if isinstance(words, dict) else list(words)
            
            if length:
                # Filter ketat: hanya kata dengan panjang yang diminta
                valid_words = [w.strip().lower() for w in word_list if w.strip().isalpha() and len(w.strip()) == length]
            else:
                # Filter default untuk mode lain (4-7 huruf)
                valid_words = [w.strip().lower() for w in word_list if w.strip().isalpha() and 4 <= len(w.strip()) <= 7]
            
            # Jika tidak ditemukan kata yang sesuai, gunakan kata default "cajel" agar tidak error
...
def setup(bot, data):
    ...
    async def send_game_menu(m):
        pesan = (
            ...
            "Pilih mode permainan, Paduka! Cukup ketik jawabanmu langsung tanpa perlu reply!\n\n"
    ...
    async def handle_game_callbacks(call):
        ...
        # Logika pemuatan kata dengan filter yang benar
