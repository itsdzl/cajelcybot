...
async def chat_gpt_text(prompt, prompt_messages):
    ...
    response = Obj()
    response.choices = [Obj()]
    response.choices[0].message = Obj()
    response.choices[0].message.content = gemini_response.text

    return response

async def messages(first, last_answer_id, language, prompt_messages):
    if language == 'id':
        pr = Name + ', bayangkan kamu membaca pesan chat berikut. Buat komentar singkat dalam bahasa Indonesia yang diawali *!* dan diakhiri *!*. Pesan chat: '
    elif language == 'ru':
        ...
        if (message.pinned == False)&(message.action is None)&(str(sender.username) != str(botname[1:])):
            if (botname in message.text)&(message.id > last_answer_id)&(message.id == first):
                ...
                if language == 'ru':
                    ...
                    pr = Name + ', jawab dalam bahasa Indonesia. ' + sub
                message_id = message.id
                break
